...
<script type="text/javascript">
    ...
    function drawCharts() {
        fetch('${CP}/chart/chartData3.do')
            ...
            .then(data => {
                const deathData = data.map(item => [item.HOUR_RANGE, item.TOTAL_DEATHS || 0]);
                const casualtiesData = data.map(item => [item.HOUR_RANGE, item.TOTAL_CASUALTIES || 0]);
                const seriouslyData = data.map(item => [item.HOUR_RANGE, item.TOTAL_SERIOUSLY || 0]);
